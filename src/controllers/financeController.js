...
const myFinance = async (req, res) => {
  try {
    ...
    const [transactions, total, walletBalance] = await Promise.all([
      prisma.transaction.findMany({
        where: { sellerId: req.user.id },
        orderBy: { createdAt: 'desc' }, take, skip
      }),
      prisma.transaction.count({ where: { sellerId: req.user.id } }),
      walletService.getBalance(prisma, req.user.id)
    ]);
...
const platformFinance = async (req, res) => {
  try {
    ...
    try {
      ...
      adminWalletBalance = await walletService.getBalance(prisma, platformAdmin.id);
    } catch {
      // Nenhum admin configurado ainda — não bloqueia o resto do overview.
    }
