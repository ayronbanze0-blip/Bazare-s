...
const submit = async (req, res) => {
  try {
    ...
    if (description.trim().length < 10) return badRequest(res, 'Descrição demasiado curta (mínimo 10 caracteres).');
    if (reason.trim().length < 3) return badRequest(res, 'Motivo demasiado curto.');
