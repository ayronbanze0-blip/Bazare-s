...
// ─── GET /api/bazars/:idOrSlug/follow-status ───────────────────────
// Estado de "segue/não segue" + contagem de seguidores, sem alternar
// nada (usado por my-bazar.html para mostrar o cartão de seguidores
// sem depender só do valor que já veio no objecto do bazar).
const getFollowStatus = async (req, res) => {
  try {
    ...
    logger.error(`[Bazar.getFollowStatus] ${err.message}`);
    return serverError(res);
  }
};

// ─── GET /api/bazars/ranking ────────────────────────────────────────
// Top vendedores do mês corrente, por encomendas ENTREGUE. Reutiliza
// getBadgeTier() — a mesma régua de medalhas Bronze/Prata/Ouro do
// painel "Meu Bazar" — para que o ranking e o cartão de medalha do
// vendedor mostrem sempre o mesmo nível.
const MONTH_LABELS = ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'];

const ranking = async (req, res) => {
  try {
    const start = startOfMonth(new Date());
    const period = { label: MONTH_LABELS[start.getMonth()] };

    const grouped = await prisma.order.groupBy({
      by: ['bazarId'],
      where: { status: 'ENTREGUE', deliveredAt: { gte: start } },
      _count: { _all: true },
      orderBy: { _count: { bazarId: 'desc' } },
      take: 50
    });

    if (!grouped.length) return ok(res, { ranking: [], period });

    const bazars = await prisma.bazar.findMany({
      where: { id: { in: grouped.map(g => g.bazarId) }, active: true },
      select: {
        id: true, name: true, slug: true, logoUrl: true,
        seller: { select: { name: true, verifiedSeller: true } }
      }
    });
    const byId = Object.fromEntries(bazars.map(b => [b.id, b]));

    const list = grouped
      .filter(g => byId[g.bazarId])
      .map((g, idx) => {
        const b = byId[g.bazarId];
        const totalEncomendas = g._count._all;
        return {
          rank: idx + 1,
          bazar: { slug: b.slug, name: b.name, logoUrl: b.logoUrl },
          name: b.seller?.name || '',
          verifiedSeller: !!b.seller?.verifiedSeller,
          badge: getBadgeTier(totalEncomendas),
          totalEncomendas
        };
      });

    return ok(res, { ranking: list, period });
  } catch (err) {
    logger.error(`[Bazar.ranking] ${err.message}`);
    return serverError(res);
  }
};

module.exports = { list, getOne, create, update, myBazar, trackWhatsappClick, toggleFollow, getFollowStatus, ranking };
